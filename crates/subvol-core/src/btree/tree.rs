...
impl Btree {
    ...
    /// 更新条目（entry_type=0 插入，entry_type=1 删除）
    ///
    /// 对应 bcachefs `do_bch2_trans_commit` (commit.c:1291) +
    /// `btree_key_can_insert` (commit.c:427) 检查 + split_leaf retry 机制
    ///
    /// 当 bset 满或已写入时自动创建新层（可能触发合并）。
    /// 调用者应调用 flush_pending_writes 写入设备。
    ///
    /// 当合并后数据超 NODE_SIZE 时返回 CowNeeded，自动分裂节点后重试。
    ///
    /// This legacy direct mutation entry point is retained only for the
    /// standalone structural tests. Runtime callers must use
    /// `BtreeTrans::iter` + `BtreeTrans::update` + `commit` so journal and
    /// transaction locks cover every logical update.
    #[cfg(test)]
    pub fn update(&mut self, entry: BtreeEntry) -> Result<(), StorageError> {
        let pos = entry.pos;

        if entry.entry_type == 1 {
            let leaf = if self.root.level == 0 {
                &mut self.root
            } else {
                let idx = self.find_leaf_idx(&pos)?;
                &mut self.child_nodes[idx]
            };
            leaf.lock_write_blocking();
            if entry.payload.is_empty() {
                match leaf.remove_key(&pos) {
                ...
                if let Err(err) = leaf.insert_key(entry) {
                    leaf.unlock_write();
                    return Err(err);
                }
            }
        ...
        crate::log_verbose!(
            "btree_update: id={} 插入 ino={} off={} snap={} level={} root_level={}",
            self.btree_id.0,
            pos.inode,
            pos.offset,
            pos.snapshot,
        ...
        'retry_split: loop {
            if self.root.level > 0 {
                let idx = self.find_leaf_idx(&pos)?;
                if self.child_nodes[idx].is_full() {
                    if self.root.is_full() {
                        self.split_root()?;
                        continue 'retry_split;
                    }
                    self.split_leaf(idx, &pos)?;
                    continue 'retry_split;
                }
        ...
        match leaf.try_rotate_bset() {
            Ok(_) => {}
            Err(StorageError::CowNeeded(_)) => {
                leaf.unlock_write();
                if self.root.level == 0 {
                    self.split_root()?;
                } else {
        ...
        match leaf.insert_key(entry.clone()) {
            Ok(()) => {
                leaf.unlock_write();
                Ok(())
            }
...
mod tests {
    ...
    fn insert_k(tree: &mut Btree, offset: u64, payload: u8) {
        let entry = BtreeEntry {
            btree_type: BTREE_ID_ALLOC.0,
            level: 0,
            entry_type: 0,
            pos: Bpos {
                inode: 0,
    ...
    fn test_single_entry() {
        let dev = make_dev();
        let mut tree = Btree::new(BTREE_ID_ALLOC, &dev);
        insert_k(&mut tree, 100, 42);
    ...
    #[test]
    fn test_iterates_manually_constructed_two_level_tree() {
        let dev = make_dev();
        let mut tree = Btree::new(BTREE_ID_ALLOC, &dev);
        let mut left = BtreeNode::new_leaf(BTREE_ID_ALLOC);
        let mut right = BtreeNode::new_leaf(BTREE_ID_ALLOC);
        for offset in 0..10 {
            let entry = BtreeEntry {
                btree_type: BTREE_ID_ALLOC.0,
                level: 0,
            ...
            if offset < 5 {
                left.insert_key(entry).unwrap();
            } else {
                right.insert_key(entry).unwrap();
            }
        }
        tree.child_nodes.push(left);
        tree.child_nodes.push(right);
        let mut internal = BtreeNode::new(1, BTREE_ID_ALLOC);
        for (offset, child) in [(0, 0usize), (5, 1usize)] {
            internal
                .insert_key(BtreeEntry {
                    ...
                    entry_type: ENTRY_TYPE_BTREE_PTR,
                    pos: Bpos { inode: 0, offset, snapshot: 0 },
                    payload: BtreePtr { offset: child as u64, child_level: 0 }.to_bytes(),
                })
                .unwrap();
        }
        tree.child_nodes.push(internal);
        tree.root = BtreeNode::new(2, BTREE_ID_ALLOC);
        tree.root
            .insert_key(BtreeEntry {
                btree_type: BTREE_ID_ALLOC.0,
                level: 2,
                entry_type: ENTRY_TYPE_BTREE_PTR,
                pos: Bpos { inode: 0, offset: 0, snapshot: 0 },
                payload: BtreePtr { offset: 2, child_level: 1 }.to_bytes(),
            })
            .unwrap();

        for offset in [0, 4, 5, 9] {
            let mut iter = BtreeIter::new(
                &tree,
                Bpos { inode: 0, offset, snapshot: 0 },
            );
            assert_eq!(iter.next().map(|entry| entry.pos.offset), Some(offset));
        }
    }

    #[test]
    fn test_sparse_inode_iteration_advances_to_next_leaf() {
        let dev = make_dev();
        let mut tree = Btree::new(BTREE_ID_ALLOC, &dev);
        let mut left = BtreeNode::new_leaf(BTREE_ID_ALLOC);
        let mut right = BtreeNode::new_leaf(BTREE_ID_ALLOC);
        for (node, inode) in [(&mut left, 7), (&mut right, 100)] {
            node.insert_key(BtreeEntry {
                btree_type: BTREE_ID_ALLOC.0,
                level: 0,
                entry_type: 0,
                pos: Bpos { inode, offset: 0, snapshot: 0 },
                payload: vec![inode as u8],
            })
            .unwrap();
        }
        tree.child_nodes.extend([left, right]);
        tree.root = BtreeNode::new(1, BTREE_ID_ALLOC);
        for (inode, child) in [(7, 0usize), (100, 1usize)] {
            tree.root
                .insert_key(BtreeEntry {
                    btree_type: BTREE_ID_ALLOC.0,
                    level: 1,
        ...
        let mut iter = BtreeIter::new(&tree, Bpos::MIN);
        assert_eq!(iter.next().map(|entry| entry.pos.inode), Some(7));
        assert_eq!(iter.next().map(|entry| entry.pos.inode), Some(100));
        assert!(iter.next().is_none());
    }

    #[test]
    fn test_direct_insert_splits_variable_size_root() {
        let dev = make_dev();
        let mut tree = Btree::new(BTREE_ID_ALLOC, &dev);
        for offset in 0..3_000 {
            insert_k(&mut tree, offset, (offset & 0xff) as u8);
        }
    ...
    fn test_forward_iteration() {
        ...
        let mut tree = Btree::new(BTREE_ID_ALLOC, &dev);
        for i in 0..10 {
            insert_k(&mut tree, i * 100, i as u8);
        }
        ...
        for i in 0..10 {
            ...
            assert_eq!(entry.pos.offset, i * 100);
    ...
    fn test_prev_iteration() {
        ...
        assert_eq!(entry.pos.offset, 400);

        let prev = iter.prev();
        assert!(prev.is_some());
        assert_eq!(prev.unwrap().pos.offset, 300);
    }
    ...
    fn test_seek_reset() {
        ...
        assert_eq!(entry.pos.offset, 500);
    }
    ...
    fn test_next_entry_prev_entry() {
        ...
        // BtreeNode::next_entry/prev_entry 直接在节点上使用
        let next = tree.root.next_entry(&Bpos {
            inode: 0,
            offset: 100,
            snapshot: 0,
        });
        assert!(next.is_some());
        assert_eq!(next.unwrap().pos.offset, 200);

        let prev = tree.root.prev_entry(&Bpos {
            inode: 0,
            offset: 300,
            snapshot: 0,
        });
        assert!(prev.is_some());
        assert_eq!(prev.unwrap().pos.offset, 200);
    }
    ...
    fn test_first_last_entry() {
        ...
        // BtreeNode::first_entry/last_entry 直接在节点上使用
        let first = tree.root.first_entry();
        assert!(first.is_some());
        assert_eq!(first.unwrap().pos.offset, 0);

        let last = tree.root.last_entry();
        assert!(last.is_some());
        assert_eq!(last.unwrap().pos.offset, 400);
    }
    ...
    fn test_peek_upto() {
        ...
        let entry = iter.peek_upto(Bpos {
            inode: 0,
            offset: 500,
            snapshot: 0,
        });
        assert!(entry.is_some());
        assert_eq!(entry.unwrap().pos.offset, 200);
        ...
        assert!(entry.is_none(), "end 为 200 时 entry.pos=200 不满足 < 200");
    ...
    fn test_advance_between_entries() {
        ...
        assert_eq!(entry.pos.offset, 0);

        assert!(iter.advance(), "advance at offset 0 should succeed");
        let entry = iter.next().unwrap();
        assert_eq!(
            entry.pos.offset, 100,
            "after advance, next skips to offset 100"
        );

        assert!(iter.advance(), "advance at offset 100 should succeed");
        let entry = iter.next().unwrap();
        assert_eq!(
            entry.pos.offset, 200,
            "after second advance, next skips to offset 200"
        );
    }
    ...
    fn test_advance_then_rewind_cycle() {
        ...
        // advance: pos=100 → pos=101 (skips past entry 100)
        assert!(iter.advance());
        // peek_upto with end=200 should find nothing since pos=101 is past entry at 100
        // but < 200, so peek_upto returns entry at 200
        let entry = iter.peek_upto(Bpos {
            inode: 0,
            offset: 300,
            snapshot: 0,
        ...
        assert!(entry.is_some());
        assert_eq!(
            entry.unwrap().pos.offset,
            200,
            "after advance, peek_upto should see next entry"
        );

        // rewind: pos=101 → pos=100 (back to where we were)
        assert!(iter.rewind());
        let entry = iter.peek_upto(Bpos {
            inode: 0,
            offset: 150,
        ...
        assert_eq!(
            entry.unwrap().pos.offset,
            100,
            "after rewind, peek_upto should see original entry"
        );
    }
    ...
    fn test_traverse() {
        ...
        iter.traverse();
        let entry = iter.next().unwrap();
        assert_eq!(entry.pos.offset, 200, "traverse + next 应找到新位置");
    }
